...
function handleDependencyErrors(callback) {
  return function(error) {
    if(error && argv.skipDependencyCheck) {
...
function checkGithubConnection(callback) {
  ...
  axios.get(url, { headers: { 'User-Agent': DEFAULT_USER_AGENT } })
    .then(() => callback(null))
    .catch(() => callback(`Failed to connect to ${url}`));
}
...
function checkLatestAdaptRepoVersion(repoName, versionLimit, callback) {
  ...
  var _getReleases = function(done) {
    ...
    axios.get(nextPage, { headers, validateStatus: null })
      .then(response => done(null, response))
      .catch(error => done(error));
  };
  var _requestHandler = function(error, response) {
    if(response) {
    ...
    try {
      var releases = (response.data || []).sort((a, b) => {
...
function updateRepo(opts, callback) {
  fetchRepo(opts, function(error) {
    ...
    checkOptions(opts, 'update', function(error) {
      ...
      execCommand(`git pull && git reset --hard && git checkout ${opts.revision}`, {
        cwd: opts.directory
      }, function(error) {
        hideSpinner();
        if(error) {
...
function updateFrameworkPlugins(opts, callback) {
  ...
  fs.readJSON(path.join(opts.directory, 'adapt.json'), function(error, json) {
    ...
    async.eachSeries(plugins, function(plugin, pluginCallback) {
      ...
      const pluginSource = json.dependencies[plugin];

      const onInstallComplete = error => {
        hideSpinner();
      ...
      const isGitUrl = /^(git\+|http|\w+:\/\/)|(\.git)/i.test(pluginSource);
      if (isGitUrl) {
        app.bowermanager.installFromGitUrl(plugin, pluginSource, onInstallComplete);
      } else {
        app.bowermanager.installLatestCompatibleVersion(plugin, onInstallComplete);
      }
    }, function(error) {
      hideSpinner();
      if(error) {
